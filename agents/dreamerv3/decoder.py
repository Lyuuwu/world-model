"""
agents/dreamerv3/decoder.py

DreamerV3 Decoder — 從 RSSM latent state 重建 observations

════════════════════════════════════════════════════════════════
核心概念 (論文 Eq.1)
════════════════════════════════════════════════════════════════

    x̂_t ~ p_ϕ(x̂_t | h_t, z_t)

Decoder 接收 RSSM 的 model state feat = cat(deter, stoch.flatten())
重建所有 observation keys。它是 prediction loss L_pred 的主要組件。

════════════════════════════════════════════════════════════════
架構總覽：雙路徑設計（與 Encoder 對稱）
════════════════════════════════════════════════════════════════

  feat (h_dim + stoch*classes)
    │
    ├──── [Image Path] ─── spatial projection ─── CNN upsample ─── sigmoid ─── MSE per pixel
    │
    └──── [Vector Path] ── MLP body ─── per-key LinearHead ─── Distribution per key


兩條路徑的 output 都包裝成 Distribution 物件，統一用 .loss(target) 計算 prediction loss。

════════════════════════════════════════════════════════════════
JAX 原作設計對照 (dreamerv3_rssm.py Decoder class)
════════════════════════════════════════════════════════════════

作者的 image path 有兩種 spatial projection 模式：
  1. bspace=0 (simple):  feat → Linear(minH*minW*depths[-1]) → reshape → CNN
  2. bspace>0 (default): deter 走 BlockLinear，stoch 走 Linear×2，兩者相加

我們實作 bspace>0 的版本（default），因為這是作者實際使用的。

關鍵細節：
  - BlockLinear 的 blocks 數 = bspace (default 8)，跟 RSSM Block GRU 的 blocks 是獨立參數
  - deter 經 BlockLinear 後要 rearrange 成 (H_min, W_min, groups*C) 的 spatial 格式
  - stoch 走兩層 Linear (先 2*units 再 minH*minW*depths[-1]) 是因為 stoch 維度較小需要更多投影
  - 兩條 spatial 分支相加後過 Norm+Act，再進 CNN upsampling
"""
